...
import { fromEncodedBinaryToArrayBuffer, constantTimeEqual } from '../lib/ts/Crypto';
import { isNotNil } from '../lib/ts/util/isNotNil';
import EventTarget from '../lib/ts/textsecure/EventTarget';
...
class IdentityRecord {
  ...
  verified: number;
  nonblockingApproval: boolean;
  validationError: Error | undefined = undefined;
  ...
  isValid() {
    if (typeof this.id !== 'string') {
      this.validationError =  new Error('Invalid identity key id');
      return false;
    }
    if (!(this.publicKey instanceof ArrayBuffer)) {
      this.validationError = new Error('Invalid identity key publicKey');
      return false;
    }
    if (typeof this.firstUse !== 'boolean') {
      this.validationError =  new Error('Invalid identity key firstUse');
      return false;
    }
    if (typeof this.timestamp !== 'number' || !(this.timestamp >= 0)) {
      this.validationError = new Error('Invalid identity key timestamp');
      return false;
    }
    if (!validateVerifiedStatus(this.verified)) {
      this.validationError = new Error('Invalid identity key verified');
      return false;
    }
    if (typeof this.nonblockingApproval !== 'boolean') {
      this.validationError = new Error('Invalid identity key nonblockingApproval');
      return false;
    }
    return true;
  }
...
type IdentityKeyType = IdentityKeyDataType & {
  id: string;
};
...
export class SignalProtocolStore extends EventTarget {
  ...
  async getIdentityKeyPair(): Promise<KeyPairType> {
    if (!this.ourIdentityKey) {
      throw new Error('No identity key defined.')
    }
    return this.ourIdentityKey;
  }

  async getLocalRegistrationId(): Promise<number> {
    if (!this.ourRegistrationId) {
      throw new Error('No registration ID defined.')
    }
  ...
  async removePreKey(keyId: number): Promise<void> {
    ...
    try {
      this.dispatchEvent(new Event('removePreKey'));
    } catch (error) {
      window.log.error(
  ...
  async saveIdentityWithAttributes(
    encodedAddress: string,
    attributes: IdentityKeyDataType
  ): Promise<void> {
    if (encodedAddress === null || encodedAddress === undefined) {
      throw new Error('Tried to put identity key for undefined/null key');
    }
  ...
  updateUnprocessedsWithData(items: Array<Partial<UnprocessedType>>): Promise<void> {
  ...
  on(key: string, callback: () => void): any {
    return this.addEventListener(key, callback)
  }
}
