...
function createCertificateValidator(trustRoot) {
  return {
    ...
    async validate(certificate, validationTime) {
      ...
      await libsignal._curve.libsignal_Curve_async.verifySignature(
        trustRoot,
...
function _createUnidentifiedSenderMessageContentFromBuffer(serialized) {
  const TypeEnum = UnidentifiedSenderMessage.Message.Type;
...
function _createUnidentifiedSenderMessageContent(
  ...
  const innerMessage = new UnidentifiedSenderMessage.Message();
  innerMessage.type = _getProtoMessageType(type);
  innerMessage.senderCertificate = SenderCertificate.decode(
...
class SecretSessionCipher {
  ...
  async encrypt(destinationAddress, senderCertificate, paddedPlaintext) {
    ...
    const ephemeral = await libsignal._curve.libsignal_Curve_async.generateKeyPair();
  ...
  async _calculateEphemeralKeys(ephemeralPublic, ephemeralPrivate, salt) {
    const ephemeralSecret = await libsignal._curve.libsignal_Curve_async.calculateAgreement(
      ephemeralPublic,
      ephemeralPrivate
    );
  ...
  async _calculateStaticKeys(staticPublic, staticPrivate, salt) {
    const staticSecret = await libsignal._curve.libsignal_Curve_async.calculateAgreement(
      staticPublic,
      staticPrivate
    );
